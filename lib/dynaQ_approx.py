...
def approx_dyna_q(env, init_q, alpha, num_steps, n, num_of_episodes=None):
    ...
    Q0 = tc = TileCodingGridWorldWItems(np.array([0, 0]), np.array([env.m, env.m]), num_tilings, tile_width, env.k,
                                   env.n * env.m,
                                   calc_tile_width=True)
    Q1 = tc = TileCodingGridWorldWItems(np.array([0, 0]), np.array([env.m, env.m]), num_tilings, tile_width, env.k,
                                   env.n * env.m,
                                   calc_tile_width=True)
    Q2 = tc = TileCodingGridWorldWItems(np.array([0, 0]), np.array([env.m, env.m]), num_tilings, tile_width, env.k,
                                   env.n * env.m,
                                   calc_tile_width=True)
    Q3 = tc = TileCodingGridWorldWItems(np.array([0, 0]), np.array([env.m, env.m]), num_tilings, tile_width, env.k,
                                   env.n * env.m,
                                   calc_tile_width=True)

    # Q0 is the approximation for q[s][0], etc..
    Q_approximations = [Q0, Q1, Q2, Q3]
    ...
    start_features = (env.grid_cell,env.items_status,env.cell_visit_status)
    for i in range(0,num_steps):
        S = grid_world.state
        A = pi.action(S)
        previously_visited.append((S, A))
        (grid_cell, R, items_status, cell_visit_status, final) = grid_world.step(A)
        #model.update(S,A,SP,R,final)
        #print(S)
        #print(SP)
        G = R + gamma* max([Q0(grid_cell,items_status,cell_visit_status),
                           Q1(grid_cell,items_status,cell_visit_status),
                           Q2(grid_cell,items_status,cell_visit_status)
                           Q3(grid_cell,items_status,cell_visit_status)])
        Q.update(alpha,G,)
